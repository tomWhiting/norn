...
async fn execute(cli: &Cli) -> Result<ExitCode, PrintError> {
    ...
    let parts = assemble_print_agent(cli)
        .await
        .map_err(|err| fail_before_assembly(cli, err))?;
...
pub(super) async fn orchestrate(
    ...
    // Captured before the run consumes `parts`: an error envelope names
    // the model and session the failed run was assembled with (R3 keeps
    // the payload minimal; the nullable fields stay null only when
    // genuinely unknown, i.e. pre-assembly).
    let model = parts.model.clone();
    let session_id: Option<String> = parts.session_entry.as_ref().map(|entry| entry.id.clone());
    let is_driven = driven_run.is_some();

    let result = orchestrate_run(cli, parts, prompt, output_schema, driven_run).await;
