...
fn fixture() -> TestResult<AppState> {
    ...
    for (item, reference) in state.screen.conversation.demands.clone() {
        let demand = state
            .transcript
...
fn retired_revision_reflows_visible_bytes_and_keeps_copy_scopes_distinct() -> TestResult {
    ...
    assert!(state.screen.conversation.hit_rows.is_empty());
    assert!(state.screen.conversation.demands.is_empty());
    ...
    assert!(state.screen.conversation.reading_snapshot.is_none());
...
fn failed_publication_cannot_replace_snapshot_and_source_change_retires_it() -> TestResult {
    ...
    assert!(state.screen.conversation.prepared_reading.is_none());
    let source = EventStore::new().bind_view_source(
        &SessionBinding::ephemeral_root(),
        Uuid::new_v4(),
        None,
