...
async fn finish_history(view: &mut Transcript) -> TestResult {
    let result = view
        .history_tasks
        .join_next()
        .await
...
async fn missing_and_foreign_readers_do_not_poison_pending_demands() -> TestResult {
    ...
    assert!(view.load_older().is_err());
    assert!(view.history_tasks.is_empty());
    view.request_latest();
    assert!(view.load_latest().is_err());
...
async fn missing_body_access_can_retry_after_attaching_the_actual_reader() -> TestResult {
    ...
    assert!(view.load_body(&id, &reference, false).is_err());
    assert!(view.body_tasks.is_empty());
    view.attach_history_reader(open_history_reader(store).await?)?;
    view.load_body(&id, &reference, false)?;
    view.load_body(&id, &reference, false)?;
    assert_eq!(view.body_tasks.len(), 1);
    let result = view
        .body_tasks
        .join_next()
        .await
        .ok_or("missing body task")?;
    ...
    view.load_body(&id, &reference, true)?;
    let result = view
        .body_tasks
        .join_next()
        .await
...
async fn independent_views_keep_their_readers_through_new_appends_and_foreign_refusal() -> TestResult
{
    ...
    assert!(root_view.load_older()?);
    assert!(child_view.load_older()?);
    // The selected view may change while either task is running: each queue has one owner.
    finish_history(&mut child_view).await?;
    finish_history(&mut root_view).await?;
