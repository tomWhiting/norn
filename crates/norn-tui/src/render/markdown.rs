...
impl MarkdownRenderer {
    ...
    ///
    /// Used by the dispatch layer's the former dim-state helper helper before a
    ...
    /// parser-state reset.
    ///
    /// the former dim-state helper: `crate::app::helpers::clear_dim_state`
    ...
    ///
    /// # Errors
    /// Propagates syntax errors. The accepted chunk remains buffered on failure;
    /// callers retain their original source and must not submit the same chunk again.
    pub fn feed(&mut self, chunk: &str) -> Result<FeedOutput, SyntaxError> {
    ...
    ///
    /// # Errors
    /// Propagates syntax errors without consuming pending text or changing dim state.
    pub fn finalize(&mut self) -> Result<FeedOutput, SyntaxError> {
        if self.pending.is_empty() {
            return Ok(FeedOutput::empty());
        }
        let scan = scan_pending(&self.pending);
    ...
    fn try_flush_styled(&mut self) -> Result<String, SyntaxError> {
        ...
        if safe_end > 0 {
            ...
            let rendered = self.render_segment(&to_render)?;
    ...
    fn render_segment(&mut self, source: &str) -> Result<String, SyntaxError> {
        if source.is_empty() {
            return Ok(String::new());
        }
...
mod tests {
    ...
    #[test]
    fn feed_failure_keeps_accepted_fence_and_prior_paragraph_state()
    -> Result<(), Box<dyn std::error::Error>> {
        let mut renderer = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let prior = renderer.feed("*Prior paragraph.*\n\n")?;
        assert!(prior.styled.contains("Prior paragraph."));
        renderer.highlighter = crate::render::syntax::tests::failing_highlighter()?;
        let paragraph_pending = renderer.paragraph_pending;
        assert!(paragraph_pending);
        let dim_active = renderer.dim_active;
        let original = "```failure\nsafe\n!\n```\n";
        assert!(matches!(
            renderer.feed(original),
            Err(SyntaxError::Highlight { offset: 5, .. })
        ));
        assert_eq!(renderer.pending, original);
        assert_eq!(renderer.paragraph_pending, paragraph_pending);
        assert_eq!(renderer.dim_active, dim_active);

        renderer.highlighter = SyntaxHighlighter::new();
        let recovered = renderer.finalize()?;
        assert!(strip_ansi(&recovered.styled).contains("safe\n!\n"));
        assert!(renderer.pending.is_empty());
        assert_eq!(renderer.finalize()?.styled, "");
        Ok(())
    }

    #[test]
    fn finalize_failure_keeps_pending_text_and_dim_state_for_explicit_recovery()
    -> Result<(), Box<dyn std::error::Error>> {
        let mut renderer = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        renderer.highlighter = crate::render::syntax::tests::failing_highlighter()?;
        let original = "```failure\nsafe\n!\n```";
        let initial = renderer.feed(original)?;
        assert!(initial.styled.is_empty());
        let dim_active = renderer.dim_active;
        let paragraph_pending = renderer.paragraph_pending;
        assert!(matches!(
            renderer.finalize(),
        ...
        renderer.highlighter = SyntaxHighlighter::new();
        let recovered = renderer.finalize()?;
        assert_eq!(recovered.replace_dim, dim_active);
        assert!(strip_ansi(&recovered.styled).contains("safe\n!\n"));
        assert!(renderer.pending.is_empty());
        Ok(())
    }
    ...
    fn collect_styled(r: &mut MarkdownRenderer, input: &str) -> Result<String, SyntaxError> {
        let out = r.feed(input)?;
        let tail = r.finalize()?;
        Ok(format!("{}{}", out.styled, tail.styled))
    }
    ...
    #[test]
    fn bold_emits_bold_sgr() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn italic_emits_italic_sgr_when_supported() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn italic_falls_back_to_underline() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let out = collect_styled(&mut r, "*emph*")?;
    ...
    #[test]
    fn inline_code_uses_distinct_foreground() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn nested_bold_italic_emits_both_attributes() -> Result<(), Box<dyn std::error::Error>> {
        let caps = caps_with(|c| c.italic_support = true);
        let mut r = MarkdownRenderer::new(caps, 80);
    ...
    #[test]
    fn heading_emits_bold_and_text() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn unordered_list_uses_bullet_and_indent() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn ordered_list_numbers_sequentially() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn horizontal_rule_spans_terminal_width() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn link_falls_back_to_bracketed_text() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn link_emits_osc8_when_supported() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn fenced_code_block_baseline_uses_palette() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let out = collect_styled(&mut r, "```rust\nfn main() {}\n```\n")?;
    ...
    #[test]
    fn unlabeled_code_block_falls_back_via_first_line() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let out = collect_styled(&mut r, "```\n#!/bin/bash\necho hi\n```\n")?;
        assert!(!out.is_empty());
    ...
    #[test]
    fn code_fence_split_across_chunks_buffers_until_close() -> Result<(), Box<dyn std::error::Error>>
    {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn unclosed_fence_on_finalize_renders_plain() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let first = r.feed("```rust\nfn main() {\n")?;
        let tail = r.finalize()?;
    ...
    #[test]
    fn split_bold_across_chunks() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn split_italic_across_chunks() -> Result<(), Box<dyn std::error::Error>> {
        let caps = caps_with(|c| c.italic_support = true);
        let mut r = MarkdownRenderer::new(caps, 80);
    ...
    #[test]
    fn plain_text_before_open_marker_dims_immediately() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn unclosed_marker_buffers_styled_until_finalize() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("**")?;
        for _ in 0..50 {
            let out = r.feed("x")?;
    ...
    #[test]
    fn empty_feed_returns_empty() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let out = r.feed("")?;
    ...
    #[test]
    fn finalize_with_no_pending_returns_empty() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("hello\n")?;
        r.finalize()?;
        let tail = r.finalize()?;
    ...
    #[test]
    fn single_paragraph_has_no_trailing_newline() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn paragraph_break_carried_across_segments() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn paragraph_break_before_plain_text_across_segments() -> Result<(), Box<dyn std::error::Error>>
    {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn two_paragraphs_separated_by_double_newline() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn streaming_tokens_produce_no_spurious_newlines() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn dim_preview_returned_for_inline_tokens() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn dim_replaced_on_newline_with_styled() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("Hello ")?;
    ...
    #[test]
    fn dim_suppressed_inside_code_fence() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("```rust\n")?;
    ...
    #[test]
    fn finalize_replaces_dim_with_styled() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("partial")?;
        let tail = r.finalize()?;
    ...
    #[test]
    fn feed_returns_stripped_dim_for_heading_chunk() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn feed_returns_full_pending_dim_on_each_chunk() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn clear_dim_returns_true_when_dim_was_active() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("Hello ")?;
    ...
    #[test]
    fn strikethrough_emits_sgr_9_and_29() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn strikethrough_inside_bold_emits_both_sgr() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn checked_task_list_item_uses_checked_box_glyph() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn unchecked_task_list_item_uses_unchecked_box_glyph() -> Result<(), Box<dyn std::error::Error>>
    {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn mixed_task_and_regular_list_items_render_correctly() -> Result<(), Box<dyn std::error::Error>>
    {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn inline_math_renders_with_inline_code_colour() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn display_math_indents_and_renders_with_colour() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn inline_html_renders_dim() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn block_html_renders_dim() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn fence_repair_splits_embedded_close_backtick() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("```rust\nfn x() {}```\nafter\n")?;
        r.finalize()?;
    ...
    #[test]
    fn fence_repair_no_op_when_no_open_fence() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn fence_repair_handles_tilde_fence() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        r.feed("~~~rust\nfn x() {}~~~\nafter\n")?;
        r.finalize()?;
    ...
    #[test]
    fn fence_repair_does_not_close_mismatched_kind() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn fence_repair_inserts_only_when_fence_is_open() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn nested_unordered_lists_cycle_bullet_glyphs() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn paragraph_before_list_in_same_segment_is_tight() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let out = collect_styled(&mut r, "Intro:\n\n- One\n- Two")?;
        let plain = strip_ansi(&out);
    ...
    #[test]
    fn ordered_lists_keep_numbers_at_every_depth() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn streaming_ordered_list_preserves_nested_numbers() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        let mut out = String::new();
        for chunk in [
            "1. Plan a trip\n",
    ...
    #[test]
    fn image_with_alt_renders_dim_placeholder() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn image_without_alt_renders_bare_placeholder() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn collapse_blank_lines_handles_unicode() -> Result<(), &'static str> {
        let collapsed = collapse_blank_lines("α\n\n\n\nβ").ok_or("four newlines must collapse")?;
    ...
    #[test]
    fn blockquote_nested_emits_two_prefix_units() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn blockquote_inline_formatting_renders_inside_prefix() -> Result<(), Box<dyn std::error::Error>>
    {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn blockquote_bold_spanning_soft_break_stays_bold() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
        ...
        let last_prefix = out
            .rfind(prefix)
            .ok_or("blockquote output must contain the dim prefix")?;
    ...
    #[test]
    fn table_basic_renders_with_unicode_borders() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn single_pipe_text_still_dim_previews() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 80);
    ...
    #[test]
    fn table_header_renders_bold() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 40);
    ...
    #[test]
    fn table_with_inline_formatting_preserves_styles() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 40);
    ...
    #[test]
    fn table_with_alignment_renders_without_panic() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 40);
    ...
    #[test]
    fn table_narrow_width_does_not_overflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn table_single_column_renders_without_panic() -> Result<(), Box<dyn std::error::Error>> {
        let mut r = MarkdownRenderer::new(TerminalCaps::baseline(), 40);
