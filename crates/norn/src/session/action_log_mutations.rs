...
pub(super) fn extract_mutations(
    ...
    match tool_name {
        "edit" => extract_edit(tool_call_id, output),
        "write" => extract_write(tool_call_id, output, follow_ups),
        "apply_patch" => extract_apply_patch(tool_call_id, output),
        _ => Vec::new(),
    }
}
...
fn extract_edit(tool_call_id: &str, output: &serde_json::Value) -> Vec<RecordedMutation> {
...
fn extract_write(
    ...
    let (operation, diff_stats) = match stored_before_content(follow_ups, &file_path) {
        Some(old_content) => {
...
fn extract_apply_patch(tool_call_id: &str, output: &serde_json::Value) -> Vec<RecordedMutation> {
