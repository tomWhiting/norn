...
/// Require a `Completed` result, returning a descriptive error otherwise.
#[track_caller]
fn assert_completed(result: AgentStepResult) -> TestResult<(Value, Usage)> {
    match result {
        AgentStepResult::Completed { output, usage, .. } => Ok((output, usage)),
        other => Err(format!("expected AgentStepResult::Completed, got: {other:?}").into()),
    }
}

/// Run `run_agent_step` with a `LoopContext` and the given executor, returning
/// the typed result, including any runner failure.
...
#[tokio::test]
async fn default_auto_state_threads_supported_provider() -> TestResult {
...
#[tokio::test]
async fn provider_threaded_second_step_uses_response_anchor() -> TestResult {
...
#[tokio::test]
async fn provider_threaded_tool_continuation_sends_only_tool_result() -> TestResult {
    let config = AgentLoopConfig {
        conversation_state: ConversationStateMode::ProviderThreaded,
        ..AgentLoopConfig::default()
    };
...
#[tokio::test]
async fn test_full_agent_step_with_real_file_tools() -> TestResult {
    let dir = tempdir().map_err(|error| std::io::Error::other(format!("tempdir: {error}")))?;
    ...
    tokio::fs::write(&source_path, source_content)
        .await
        .map_err(|error| std::io::Error::other(format!("write source file: {error}")))?;
    ...
    let written = tokio::fs::read_to_string(&output_path)
        .await
        .map_err(|error| {
            std::io::Error::other(format!("output file must exist on disk: {error}"))
        })?;
    assert_eq!(
...
#[tokio::test]
async fn test_agent_step_with_bash_and_search() -> TestResult {
    let dir = tempdir().map_err(|error| std::io::Error::other(format!("tempdir: {error}")))?;
    // Create a file for the search tool to find.
    let target = dir.path().join("needle.txt");
    tokio::fs::write(&target, "search me\n")
        .await
        .map_err(|error| std::io::Error::other(format!("write target file: {error}")))?;
...
#[tokio::test]
async fn test_rules_fire_on_file_write() -> TestResult {
    let rule = Rule {
...
#[tokio::test]
async fn test_hooks_block_tool_execution() -> TestResult {
    struct BlockBash;
...
#[tokio::test]
async fn test_schema_enforcement_retry() -> TestResult {
...
#[tokio::test]
async fn test_active_human_input_is_plain_user_message() -> TestResult {
    ...
    let steer_id = active_tx
        ...
        .map_err(|error| {
            std::io::Error::other(format!("active steer should be accepted: {error}"))
        })?;
    ...
    let requests = provider
        .requests()
        .map_err(|error| std::io::Error::other(format!("requests recorded: {error}")))?;
    ...
    assert!(delivery_rx.try_recv().is_some_and(|delivery| {
        delivery.id == steer_id
            && delivery.content == "human mid-turn steer"
            && store.events().iter().any(|event| {
                event.base().id == delivery.event_id
                    && matches!(event, SessionEvent::UserMessage { content, .. } if content == "human mid-turn steer")
            })
    }));
    Ok(())
}
...
#[tokio::test]
async fn test_inbound_steer_message() -> TestResult {
    let mut handlers: std::collections::HashMap<String, ToolHandler> =
        std::collections::HashMap::new();
    handlers.insert(
        "read_file".to_string(),
    ...
    .await
    .map_err(|error| std::io::Error::other(format!("send steer message: {error}")))?;
    ...
    .await
    .map_err(|error| std::io::Error::other(format!("run_agent_step must succeed: {error}")))?;
...
#[tokio::test]
async fn test_read_before_write_enforcement_through_registry() -> TestResult {
    let dir = tempdir().map_err(|error| std::io::Error::other(format!("tempdir: {error}")))?;
    let existing = dir.path().join("existing.txt");
    tokio::fs::write(&existing, "original\n")
        .await
        .map_err(|error| std::io::Error::other(format!("write existing file: {error}")))?;
    ...
    let on_disk = tokio::fs::read_to_string(&existing)
        .await
        .map_err(|error| {
            std::io::Error::other(format!("existing file must still be readable: {error}"))
        })?;
    assert_eq!(
        on_disk, "original\n",
...
#[tokio::test]
async fn test_no_schema_mode_completes_on_text_stop() -> TestResult {
    let events = vec![
...
#[tokio::test]
async fn test_rules_system_context_append_delivery() -> TestResult {
    let rule = Rule {
...
#[tokio::test]
async fn test_schema_budget_exhaustion_returns_schema_unreachable() -> TestResult {
    let turn1 = vec![
    ...
    match result {
        ...
        other => return Err(format!("expected SchemaUnreachable, got: {other:?}").into()),
    }
...
#[tokio::test]
async fn diagnostics_collector_records_schema_validation_failure_then_success() -> TestResult {
    use std::sync::Arc;
...
#[tokio::test]
async fn diagnostics_collector_records_pre_validate_block() -> TestResult {
    use std::sync::Arc;
    ...
    tokio::fs::write(&path, "preexisting\n")
        .await
        .map_err(|error| std::io::Error::other(format!("seed file: {error}")))?;
...
#[tokio::test]
async fn runtime_post_validate_check_can_push_diagnostics_via_extension() -> TestResult {
    use std::sync::Arc;
