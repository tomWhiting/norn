...
mod tests {
    ...
    #[test]
    fn resolve_codex_home_rejects_relative_override() -> Result<(), std::io::Error> {
        let result = resolve_codex_home(Some(std::path::PathBuf::from(".codex")));
        let error = result
            .err()
            .ok_or_else(|| std::io::Error::other("relative credential directory was accepted"))?;
