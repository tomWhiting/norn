//! Validation and runtime assembly for [`AgentBuilder`](super::AgentBuilder).

use std::sync::Arc;

use crate::agent::assembly::{
    AgentInfraParts, OverlayOverrides, RuntimeOverlay, ToolContextParts,
    apply_base_to_loop_context, assemble_tool_context, collect_tool_definitions,
...
use super::prompt::{PromptInstallParts, install_effective_prompt};
...
impl AgentBuilder {
    ...
    /// # Errors
    ///
    /// Returns [`NornError::Config`] for invalid paths, profile/model/tool
    /// settings, channel capacities, session conflicts, or incomplete agent
    /// coordination. Returns [`NornError::Session`] when managed-session
    /// creation, resume, or fork fails, and [`NornError::Provider`] when a
    /// threaded provider lacks a stable identity or the session is bound to
    /// another identity.
    pub fn build(mut self) -> Result<Agent, NornError> {
        ...
        let tool_output_context_window = install_effective_prompt(
            &mut loop_context,
            &mut config_override,
            &model,
            &registry,
        ...
        // Every agent's context carries its own source-aware prompt plan:
        // `fork` reads this extension to preserve the forker's authority
        // boundaries, so the root publishes its installed plan here. The root's
