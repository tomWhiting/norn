//! The concrete [`ProcessCompletionSink`] that delivers a background process's
//! completion notice over the durable injected-message path.
//!
//! This lives in `crate::agent` — not in `crate::process` — precisely so the
//! process module stays agent-agnostic (it depends only on the sink trait). The
//! delivery algorithm is the same durable path the schedule executor uses
//! ([`crate::schedule::executor`]) and cron's wake tool contract:
//!
...
use crate::process::{ProcessCompletion, ProcessCompletionSink};
...
/// Delivers a process's completion notice to its owning agent over the durable
/// injected-message path. Every handle is a cheap `Arc`/clone captured at
/// assembly.
pub struct ProcessCompletionDelivery {
...
impl ProcessCompletionDelivery {
...
impl ProcessCompletionSink for ProcessCompletionDelivery {
...
mod tests {
    ...
    fn delivers_over_a_live_inbound_channel_as_a_nil_sender_steer() {
        ...
        sink.deliver(completion("p1", Some(0), false));
    ...
    fn queues_durably_without_a_live_channel_with_a_queued_audit() {
        ...
        sink.deliver(completion("p2", None, true));
    ...
    fn killed_disposition_is_distinct_from_a_normal_exit() {
        ...
        sink.deliver(completion("p3", None, true));
        let drained = rx.drain();
        let payload: serde_json::Value = serde_json::from_str(&drained[0].content).unwrap();
    ...
    async fn real_process_completion_delivers_a_steer_over_a_live_channel() {
        ...
        let agent_id = Uuid::new_v4();
        let (tx, mut rx) = inbound_channel(8);
        let sink: Arc<dyn ProcessCompletionSink> = Arc::new(delivery(
            agent_id,
            Some(tx),
            Arc::new(PendingAgentMessages::new()),
            Arc::new(EventStore::new()),
    ...
    async fn idle_child_completion_queues_and_wakes() {
        ...
        let pending = Arc::new(PendingAgentMessages::new());
        let sink = ProcessCompletionDelivery {
            agent_id: child,
            inbound: None,
    ...
    async fn completion_wakes_a_lingering_agent() {
        ...
        let sink: Arc<dyn ProcessCompletionSink> = Arc::new(ProcessCompletionDelivery {
            agent_id,
            inbound: Some(tx),
