...
/// carrying the parent's base system instruction.
///
/// Published by the orchestrator when assembling the agent loop's
/// [`ToolContext`](crate::tool::context::ToolContext) so the fork tool can
/// compose the child's base system
/// instruction as `FORK_SYSTEM_PREAMBLE + parent_base` (R5). Absent
/// extensions are not an error — the child falls back to just the preamble.
#[derive(Clone, Debug)]
...
/// Combine the verbatim fork preamble with the parent's base system
/// instruction (R5).
///
...
#[must_use]
pub fn combine_system_instruction(parent_base: &str) -> String {
    if parent_base.is_empty() {
        FORK_SYSTEM_PREAMBLE.to_owned()
    } else {
        format!("{FORK_SYSTEM_PREAMBLE}\n\n{parent_base}")
    }
}
...
mod tests {
    ...
    fn combine_system_instruction_empty_parent_uses_preamble_only() {
        let combined = combine_system_instruction("");
    ...
    fn combine_system_instruction_prepends_preamble_to_parent_base() {
        ...
        let combined = combine_system_instruction(parent);
        assert!(
