    ...
    impl PreLlmHook for CaptureSystem {
        async fn before_llm(&self, req: &ProviderRequest) -> HookOutcome {
            let system = req
                ...
                .unwrap_or_default();
            // The managed dynamic-context Developer message is the LAST
            // message in the request (tail placement for prefix caching).
            let dynamic = req
                .messages
                .last()
...
async fn prompt_command_appears_in_dynamic_context() {
    ...
    let dynamic = snapshots[0]
        .dynamic
        .as_ref()
        .expect("managed Developer message must be present as the tail message");
    assert!(
...
#[tokio::test]
async fn queued_audit_sink_failure_fails_an_otherwise_successful_step() {
    ...
    .await
    .expect_err("the queued-audit persist failure must fail the step typed");
    assert!(
...
#[tokio::test]
async fn managed_dev_message_rides_the_tail_keeping_the_history_prefix_byte_stable() {
    use crate::rules::engine::RuleEngine;
    use crate::rules::types::{
        DeliveryMode as RDM, Rule, RuleId, TriggerCondition, TriggerTiming as TT,
    };
    ...
    for (j, msg) in prefix1.iter().enumerate() {
        let bytes1 = serde_json::to_string(msg).expect("serialize prefix1 message");
        let bytes2 = serde_json::to_string(&prefix2[j]).expect("serialize prefix2 message");
        assert_eq!(
            bytes1, bytes2,
...
async fn in_flight_compaction_leaves_one_managed_message_after_the_summary_at_the_tail() {
    ...
    let last = main.messages.last().expect("non-empty request");
    ...
    let summary_idx = summary_idx.expect("in-flight request must carry the compaction summary");
    assert!(
