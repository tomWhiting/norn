...
/// Configuration owner and compatibility facade for Norn-wrapped Claude Code.
///
/// New code should create a persistent SDK session through
/// [`spawn_session`](Self::spawn_session). [`run`](Self::run) remains the
/// synchronous, one-shot compatibility entry point.
...
impl NornWrappedClaudeCode {
    ...
    /// Run one prompt through the historical one-shot print-mode surface.
    ///
    /// This method preserves the established synchronous API. Persistent
    /// conversations, rich input, correlated controls, and safe interruption
    ...
    pub fn run(&self, prompt: &str) -> Result<Vec<SessionEvent>, IntegrationError> {
        ...
        Ok(self.capture_session_events(&claude_events))
    }
    ...
    pub(crate) fn build_command(
        ...
        self.config.build_legacy_command(prompt)
    }
}
