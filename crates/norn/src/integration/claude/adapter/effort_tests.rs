//! Claude adapter isolation, effort mapping and backend authority regressions.
...
#[test]
fn ultra_is_refused_before_spawn_on_the_claude_route() -> TestResult {
    let result = adapter().build_command(&request(Some(ReasoningEffort::Ultra)));
    match result {
        Err(ProviderError::UnsupportedFeature { feature }) => {
            assert_eq!(feature, "reasoning effort 'ultra' through Claude Runner");
        }
        Err(error) => {
            return Err(format!("expected unsupported Claude effort, got {error:?}").into());
        }
        Ok(command) => {
            return Err(format!(
                "Claude ultra unexpectedly built command: {:?}",
                command.build_args()
            )
            .into());
        }
