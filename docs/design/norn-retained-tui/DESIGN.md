...
### D-NUI003-AGENTS: Restore populated agent status through the retained frame

> **Status:** implemented_locally_verified_installed;main_205_Fable_pending

AgentStatusPanel is still populated; restore its missing retained presentation through one typed snapshot per prepared frame. Reuse existing tree::collapse/order_for_display, current five-row-plus-overflow rule, recovery/hold semantics and formatting/activity/token facts. Root-only state adds zero agent chrome. Paint compact agent/fork status rows immediately above the full-width composer within the upper display allocation; preserve tiny geometry and existing pane behavior without changing layout.rs or composer input. Accepted child activity dirties the retained frame even if the root transcript projection revision does not change. Never turn formatted ANSI into typed rows, replay agent history, infer an active agent by text, or start/restart an agent to paint status. One AgentFrame snapshot carries both the existing compact five-row-plus-overflow strip and the full admitted agent listing, including the root, for the explicitly selected Agents pane. agents::paint_pane(&AgentFrame, &mut Frame, area, scroll) clips the full list to the current pane rectangle and uses its explicit scroll; it does not reuse the compact truncation as a data cap. No second status read or second state owner. Compact root-only state still adds zero agent chrome; an explicitly opened Agents pane can show its actual root. Root integrates the chosen rectangle and sole frame writer. Preserve exact identity/order/hold/recovery and explicit unavailable facts. Listing grants no target selection/history/message authority, fabricated transcript, history read or provider activation. Within already-modified agents/status_line.rs, norn_models removes inherited test-only allow attributes and underlying unwrap/expect/panic/unwrap_err/expect_err uses structurally. Preserve every test and assertion meaning, propagate contextual fallible Result errors, and leave unrelated production behavior unchanged. The user no-bypass instruction overrides the inherited CLAUDE.md test exception; no extra source path is authorized.

### D-NUI003-READABILITY: Readable semantic rows and generated separation

> **Status:** implemented_locally_verified_installed;main_205_Fable_pending

Restore tool name beside its description with current lifecycle/duration and explicit missing-evidence wording. Insert exactly one generated blank between visible semantic items, replacing incidental empty-label gaps; no per-body-line, stream-chunk or token gaps. Use distinct BeforeItem anchors with no BodyRef, body demand or original copy/export authority. Work through indexed visible iteration/lookahead, preserve exact selection/source mappings and pinned scrolling, and reuse styled/wrapped caches without whole-history scans or per-paint reparsing. Use existing typed role/lifecycle facts: unchanged user blue RGB80/160/220, Error/failed tool red200/80/80, blocked/incomplete tool amber215/175/0, Thinking Dim+Italic merged with Markdown attributes and explicit code colors, Notice/Metadata dim. Do not search model descriptions for state words. Preserve actual source bytes, ranges, Markdown emphasis and code colors; colors supplement existing role/state words rather than being their only cue. Emit BeforeItem only when an earlier visible semantic item exists: no leading separator and no gap contributed by hidden metadata/thinking. Header remains a virtual navigation start for headerless assistant/input bodies; keep original body-cache construction and exact source/copy mappings unchanged.

### D-NUI003-BOUNDARY: Urgent repair and still-required agent interaction

> **Status:** registered_root_authorized

Preserve the retained typed view, existing frame writer, current full-width composer, original blue user markers and dim dividers, no new global header, and the existing read-only Changes split. Reuse legacy palette values; no ANSI rendering/capture/parsing roundtrip, second terminal writer, polling or provider/session-policy change. Extra files require a named amendment before edits. Select and view the main conversation, forks and subagents, then message the selected target. Restoring status alone does not fulfil this requirement. Settle exact selected-target identity, history access, operator send authority, root/fork/subagent targeting, unavailable/stale/retired handling and acknowledged send outcome; register exact source walls before implementation. No inferred authority, fake target transcript or replay of effects. Existing read-only Changes pane is available with /view pane open and /view split 2 1. Editable Iridium/Work/Session panes and selected-agent panes are separate work, not delivered by this repair. Required NUI-003 R4 separately tracks target selection/view/message and remains pending design plus exact source registration; urgent passive restoration cannot close it. channels_compatibility exclusively owns test-only migrations in tests/model_selection.rs and tests/mcp_channels.rs. Model-selection fixtures must first assert the exact refused draft is retained, then perform an explicit user clear before typing /view follow or another command; never concatenate the next command into the deliberately retained draft. MCP recovery fixture must issue the existing explicit return-to-live/follow action before waiting for an answer hidden below the pinned long-error viewport. Preserve or strengthen original model/effort/tier/budget/compaction and provider/delivery/error assertions, exact request counts and decoded ready-frame checks. Do not change product draft-retention, pinning, automatic-follow policy, runtime source or scenario meaning to satisfy old fixture assumptions.

### D-NUI003-PANE: Direct pane commands and typed auxiliary content

> **Status:** implemented_locally_verified_installed;main_205_Fable_pending

Recognize /pane as a TUI-only named entry using the existing View handler kind. /pane with no arguments toggles the existing auxiliary open state without changing selected content; /pane diff and /pane agents select the exact typed content and open it. Preserve all existing /view commands as supported aliases, help and autocomplete. Reject unknown arguments locally with LocalCommandOutcome::Rejected and preserve the draft. Both idle and active-turn paths dispatch the actual command name plus arguments through command_named; no /pane command enters provider input, steer or queue. Root adds AuxiliaryPane { Diff, Agents } and ScreenState.auxiliary in app/render.rs, defaulting to Diff to preserve the existing Changes view. Keep this content selector distinct from UpperPane, which controls conversation/auxiliary geometry focus. Root paints the selected content in the existing auxiliary rectangle and gates Changes evidence demand when Agents is selected; no new Changes body demand for agent listing. Preserve full-width composer, existing open/split/focus, narrow-terminal pane switching and current frame publication. This checkpoint keeps selected auxiliary content in current-run state. Existing saved open/split preferences remain unchanged. Persisting the new content selector through the existing frontend settings owner is required next work R6, not silently claimed here; /pane agents does not complete R4 target view/message. The existing /view pane grammar additionally accepts diff and agents as aliases for the same exact content selection. Explicit /pane diff|agents and /view pane diff|agents also set UpperPane::Changes, so the selected auxiliary content is immediately visible when narrow geometry can show only one upper pane. /pane with no arguments and /view pane toggle preserve the selected auxiliary content and focus, apart from existing visibility normalization; do not switch targets or alter transcript identity.

### D-NUI004-CONTROLS: Pane controls and one-cell divider

> **Status:** registered_root_authorized

Paint a thin one-cell vertical │ pane divider in the same Dim style as the composer separator, with no extra rows. Handle F7 pane toggle, F8 Diff, F9 Agents and F10 composer send-policy cycle through existing typed state/control owners; preserve full-width composer and existing slash equivalents. Root owns render/frame.rs and app/view_actions/keys.rs. commands.rs changes by root are HELP text only and are integrated serially by its shared-file owner. F7 preserves selected content; F8/F9 select and show exact auxiliary content including narrow UpperPane::Changes behavior, using the existing pane command semantics. No shortcut selects/messages an agent or sends provider/steer/queue input. F10 calls ComposerSendKey::next_policy() supplied by R3: Enter -> ShiftEnter -> AltEnter -> Enter. Handle one actual press; repeat/release cannot trigger repeated mutations or sends. Preserve draft, cursors, undo, existing preference edit/save handling, popup ownership and existing focus controls. The existing clickable button uses the same transition through the R2 mouse owner.

### D-NUI004-SELECTION: Displayed snapshot selection and original-source selection

> **Status:** registered_root_authorized

Ordinary dragging selects displayed text across tool headers, conversation entries and Diff rows. Prefer existing exact-source single-body copy when valid; otherwise use an immutable displayed-text snapshot. Preserve exact BodyRef selection/export separately. Never label generated Diff summaries original-file evidence. Retain the actual successfully published typed Frame (Arc ownership) for current-pointer admission; do not reconstruct text from ANSI or take a second editor/model snapshot. Failed publication revokes that display authority. On primary down capture one immutable visible-pane snapshot and grapheme-correct endpoints. Current display mappings remain tagged; there is no per-key history scan or body fetch. Keep existing exact-source selection for a valid drag confined to one mapped BodyRef. Crossing generated text/entries/Diff, or losing exact-source currency while dragging, falls back to captured displayed text and clears any misleading original-range claim. Selection remains within the chosen pane; body and generated runs form an explicit display scope. Composer keeps Iridium selection/cursor/undo ownership. Pin captured bytes and pointer mapping through streaming. Resize must never reinterpret old screen coordinates as new content; preserve completed snapshot copy or explicitly end an incompatible in-flight drag while retaining bytes. Copy feedback identifies original selected bytes versus displayed-text snapshot. Original export/search APIs continue to require their exact current original BodyRef and never silently export generated Diff text as original source. Do not add global chrome or automatic transcript bookkeeping rows. Use existing copy feedback surface. No polling, timer, arbitrary limits, provider/MCP activation or terminal-owner changes. Bind a selection gesture to typed pane identity and selected auxiliary content identity as well as the exact source/publication. Conversation, Diff and Agents are distinct display scopes even when their rectangle or common pane-focus enum is equal. Switching content or source cannot rebind an old gesture or copy authority. Original-source and displayed selection must use the same grapheme endpoint semantics for the same pointer gesture. A valid single-body selection cannot copy different graphemes merely because original versus displayed copy is chosen. Verify equality with combining marks, ZWJ emoji, wide glyphs, reverse drag and inclusive/exclusive edge cases while preserving original hard-newline rules. Revoke the old published pointer/hit authority synchronously when resize is handled, before the next queued pointer event can be admitted. Waiting until redraw/flush to revoke is insufficient. Preserve captured completed bytes as a labelled snapshot, but never reinterpret old coordinates against new geometry or a different content/source. Validate each distinct selected paint row once and retain compact selected row/range intersections. Avoid comparing/cloning the same paint row for every grapheme or allocating full body/cell copies. Preserve exact highlighted/copy ranges, explicit snapshot scope and current-source checks; measure or structurally prove the distinct-row bound in focused tests. For the current R2 review repairs, review_child_context exclusively owns app/display_selection.rs, app/view_actions/mouse.rs and app/view_actions/commands.rs as whole files. norn_tui owns display_selection_tests.rs and existing R2 PTY/oracle tests and the remaining view_actions/reading seams. Root owns the small shared render.rs revocation/publication hook and F2/keys integration. Root HELP and channels send-policy hunks go through review_child_context while this repair partition is active; no concurrent writers.

### D-NUI004-SEND: Exact Shift policy and explicit recovery

> **Status:** registered_root_authorized

tui.composer.send_key: enter | shift-enter | alt-enter /view composer send-key shift-enter; preserve enter and alt-enter ComposerSendKey::ShiftEnter in the existing frontend_preferences owner ShiftEnter submits only the delivered exact SHIFT+Enter press; ordinary Enter is always one kernel newline in that mode. Alt/Control/Meta/Super mixtures do not accidentally submit. Repeats and releases never submit. Keep the existing declared global Enter default unless the owner explicitly changes that ruling. Persist ShiftEnter as the chosen User/Local preference rather than silently migrating every installation. One next_policy() cycle: Enter -> ShiftEnter -> AltEnter -> Enter. Clickable button and F10 invoke the same value transition; preserve text/cursor/undo and existing preference save owner. Existing composer JSON projection, CAS writer, user/run/local scopes, whole-object precedence, conflict and startup validation continue unchanged. F10 cycles the same three policies on a single press only, including when Shift reporting was not detected. No provider/steer/queue admission; draft and undo survive. terminal/caps.rs detects Kitty ReportFlags; terminal/setup.rs enables DISAMBIGUATE_ESCAPE_CODES and REPORT_ALTERNATE_KEYS only after detection. map_enter already relies on delivered exact modifiers. Show Shift+Enter sends / Enter newline when Kitty disambiguation was detected and enabled. Do not equate false Kitty probe with proven impossibility: a terminal mapping may still deliver CSI-u. Show Shift+Enter unconfirmed in the same existing hint/button row; honor actually delivered SHIFT events; plain CR remains newline. Keep editing and clickable policy recovery available. No phantom Shift, no automatic Enter-send fallback, no stored preference rewrite, no new protocol probe or timeout. Root explicitly settled exact SHIFT only, no Alt fallback or CR inversion. F10 and the existing clickable policy button provide explicit recovery; false Kitty detection does not reject startup. Add pub const fn ComposerSendKey::next_policy(self) -> Self and label() spelling enter|shift-enter|alt-enter. Keep the existing declared Enter default; the selected Shift policy is saved via existing user/run/local CAS ownership and whole-tui-layer precedence. Do not silently migrate settings or serialize keyboard detection as a choice. Exact SHIFT+Enter press submits only in ShiftEnter mode; bare Enter inserts one kernel newline. Alt/Control/Meta/Super mixtures, release and repeat cannot submit accidentally. No CR inversion, inferred Shift, Alt fallback or automatic send-policy rewrite. A negative Kitty probe means unconfirmed reporting, not proof of impossibility; honor actually delivered exact SHIFT events. Existing /view composer send-key enter|shift-enter|alt-enter remains the only policy slash grammar. F10 and the clickable policy button use next_policy, preserve original draft/cursor/undo and provide explicit recovery when Shift reporting is unconfirmed. No added row or layout growth; keep existing clipped button hit areas. channels_compatibility owns frontend_preferences.rs plus the schema/composer/test paths below. Root owns keys.rs and norn_tui owns mouse.rs; send them API/hunks without competing edits. commands.rs is serialized by channels_compatibility with root HELP and norn_tui clear-selection hunks. Support retained_workspace.rs belongs to norn_tui, who integrates Shift physical-key/Kitty fixture support. Actual iridium_composer App PTYs prove idle/active exact Shift, bare newline, unchanged draft across explicit F10/click recovery, capability confirmed/unconfirmed paths, no premature request and exact admitted original bytes. Actual CLI composer_preferences restart tests prove User/Local persistence and temporary Run, preserving malformed early refusal and exact request census. Unit tests retain closed decode/null/type validation and all modifier/press/repeat/release cases.

### D-NUI004-PENDING: Required scrolling and planned display work

> **Status:** required_pending

Tool forms/raw toggle, Bash display and Markdown fenced syntax remain planned programme work. Selected-agent view/message remains NUI-003 R4. NUI-004 scrolling/latest is now registered under D-NUI004-SCROLL, with implementation and verification pending; no installation or test result is inferred.

### D-NUI004-SCROLL: Ordered logical navigation and explicit source-bound Latest

> **Status:** registered_ready_for_root_dispatch_not_verified

Preserve every terminal-delivered wheel step through ordered adjacent-compatible motion segments in the existing finite ready batch, advancing from the evolving source/width-bound logical anchor. Add a current-published-frame Latest button in the existing hint row, sharing an explicit selected-source follow/catch-up action with /view follow; no automatic repin, second writer, new timer or pixel-delta claim. Replace the single overwriteable ScrollRequest with a source-bound pending navigation plan. Capture the actual logical/published starting anchor only on the first movement; subsequent motion advances from that evolving logical position, never resets from visible.first. Compress adjacent same-direction movements for the same source/pane/geometry with checked count arithmetic. Keep direction changes and pane/source/resize/explicit-anchor changes as semantic barriers. Do not sum an entire burst into one signed net value: at the top, Up then Down must still move down, whereas a zero net sum would incorrectly leave the viewport at the top. Consume each ordered segment before final frame construction through a geometry-only cached-row advance that returns a final ViewAnchor. Count/skip cached row groups rather than allocating one LogicalRow/PaintRow per travelled row. Visible paint remains bounded by actual viewport height; body/style maps remain source/width/revision-bound. No new timer, delay, frame rate, velocity multiplier, inertia or queue cap. The plan lives only for the existing finite ready frontier and coalesces runs; it is not a persistent input queue. Preserve every reported terminal wheel step. Current one-row-per-report behavior remains the factual baseline until Tom chooses a separate explicit sensitivity setting. Non-wheel actions keep original arrival semantics. Explicit follow/anchor replacement clears or resolves earlier pending motion as part of that action, so old scroll work cannot repin the viewport after Latest. Resize keeps logical body/byte anchors, invalidates old row geometry and establishes a new geometry barrier. Source replacement cancels old motion explicitly. Wheel targets stay independent: Conversation changes its logical anchor; Diff/Agents changes its own list offset; popup selection remains popup-owned; composer wheel behavior remains existing behavior. A mixed-pane burst never merges two targets. Keep unknown history/body boundaries honest. This row does not pretend unloaded bytes are known rows or add eager whole-history/body reads. Existing explicit older/more requests remain available; any future automatic boundary demand requires its own precise registered behavior. Display the exact label ↓ Latest when the conversation viewport is pinned/not following, demonstrably away from tail, or an explicit latest catch-up is pending. Normal following tail has no button. A pin at the visible bottom may still offer resuming follow; this is deliberate. Place it right-aligned in the existing final composer hint row, with normal measured terminal-cell widths. Reserve its rectangle before clipping remaining hints. No header/footer/new row, reduced composer width or overlay over transcript text. At widths that cannot show the complete label, omit it and its hit region; /view follow remains available. When it fits, show the whole label and prevent overlap with send-key toggle, upcoming target label and other active hit regions. Never leave a clipped or hidden clickable region. The button is generated chrome with no original-body copy capability. Its hit region carries current selected source/publication identity and becomes authoritative only after terminal write+flush succeeds. Clear it on failed publication, no-paint, resize or source replacement; queued clicks cannot activate an old target. Click invokes the shared typed follow-latest action used by /view follow. It does not send text, change agent execution, steal composer focus, submit/clear the draft or silently replace selected agent. Preserve original selected-body capability for copy/export according to existing follow behavior. Integrate with NUI004 drag selection through one owner of published hit regions; button hits take precedence only inside their exact non-overlapping rectangle. Extract one follow_latest action: clear earlier pending navigation, call existing Viewport::follow_tail, mark the frame dirty, and request a source-bound catch-up outside paint. /view follow and mouse use the same path. Reuse configured history_demand and newer_history from the highest observed page cursor. One active history job at a time; if an older read is pending, retain one explicit latest request until completion rather than swallowing the click. Capture the first actual read snapshot’s accepted event frontier and catch up to it using bounded pages, with each completion yielding to the event loop. No eager whole-history clone or invented page count. When no prior history cursor exists, reuse the existing bounded initial tail demand. Preserve explicit unloaded-body continuation notices; Latest means the latest admitted conversation item, not secretly downloading every byte of a large body. An empty After result for a nonempty store must correctly prove the requested frontier has been reached; do not rely only on accept_history’s current empty-store arm. Keep sparse/unavailable coverage explicit and never label merely loaded old history as current. If the operator scrolls or changes target while catch-up is pending, accepted records may populate their valid source projection, but completion must not restore follow, steal focus, clear a newer draft or paint another source. User navigation cancels only the follow intent; normal canonical observation remains valid. Attempt dirty frame publication on history/body/input completion in the active turn loop as well as the idle loop; do not make new navigation wait for the120fps status tick. Keep the existing status cadence unchanged in this row. A later idle-timer redesign is not needed to fix the lost steps. Bind the action to the selected view/read handle when target switching is integrated. Never hardcode the root store for a button displayed over a child view. First reproduce the old failure using actual App input batching: N same-direction SGR wheel reports in one ready batch must move exactly N available rows, matching individually published reports. Run idle and during MockProvider streaming. After the semantic repair, compare fixed identical bursts and isolated steps on the same terminal size/source. Capture input receipt to successful frame publication latency, total bytes/write/flush counts, elapsed/CPU and final top anchor or numbered visible row. Report median/tail measurements without inventing a performance threshold or claiming pixel fidelity. Separate warm loaded/cache-hit scrolling, first-entry body demand, reverse movement after existing cache eviction, large tool/Markdown body and width change. Preserve actual body-map Arc identity on warm same-width steps in unit tests; show no parser/layout rebuild or body/store I/O was requested by a cache hit. Keep retained changed-cell output and one writer. Full frame preparation/changed-cell scanning and cold body reparse are candidates to measure, not established dominant costs. Do not add terminal scroll-region commands, a second writer, arbitrary overscan memory or an animation subsystem without fresh evidence. Native trackpad feel still needs a human terminal check because SGR PTY reports model terminal-delivered wheel steps, not the OS’s fractional gesture data. Preserve current API limitations honestly. R4 is isolated in /private/tmp/norn-scroll-latest on codex/norn-scroll-latest, based on f3c8309d3fd6ff0e39ef3b5aadd525f52fa2d9f8. norn_models is the sole whole-file source owner for every R4 modify/create path, including render.rs, turn/run.rs and formerly shared command/mouse/composer/test files. Root reviews and integrates after handoff; no shared writers. Historical R1-R3 ownership remains provenance and grants no concurrent editing authority in this R4 worktree. Do not touch /private/tmp/norn-pane-interaction while its full tests run. Before either app/render.rs or app/turn/run.rs would exceed500 production lines, stop that edit and propose the exact extraction file and responsibility to root/docs owner; register the named create/module-declaration wall before creation. No unspecified extraction or source path is authorized. Existing new navigation/latest modules may own their registered pure responsibilities; they cannot become unrelated catch-all modules. Before adding R4 logic, extract ScreenState definition/impl, DisplayCache, AuxiliaryPane and ScrollRequest owning state/types from app/render.rs into the registered named child app/render/screen_state.rs. render.rs retains the module declaration and type reexports at their existing paths. Preserve exact effective visibility: existing app-visible fields/types remain pub(in crate::app) from the child; render-only fields may be pub(super) there; existing public API remains public. Do not accidentally narrow app access or widen private state. Preserve state initialization, defaults, source replacement/cancellation, publication/selection revocation, cache ownership and all R1-R3 behavior. This is a structural prerequisite, not an alternate ScreenState owner or duplicated cache. Keep render.rs and every extracted production file at or below500 nonblank/noncomment lines. turn/run.rs is already near the limit: if its R4 integration would exceed500, stop and name the exact extraction path/responsibility before editing; no additional guessed file is authorized. Existing registered navigation/latest modules own only their stated responsibilities. Wheel reports target the hovered Conversation, Diff or Agents pane directly without assigning keyboard focus. Preserve the existing focused control, composer cursor visibility, draft and undo state while scrolling another pane. Explicit pointer clicks still change focus through their existing typed owner; wheel over the composer retains its existing behavior. The hover target is routing context, not keyboard focus or new source authority. Verify this distinction in actual App PTY coverage; no provider/input admission is caused by wheel routing.

### D-NUI004-PREVIEW: Truthful preview.6 metadata for verified R1-R3 only

> **Status:** registered_candidate_uninstalled

Reserve 0.1.0-preview.6 as the candidate for R1-R3 pane controls, reviewed drag selection and Shift+Enter policy. Root updates only the registered manifests and release note, then verifies and installs only after actual candidate checks. R4 scrolling/latest remains pending and is not part of this checkpoint claim. Root alone updates workspace.package.version in Cargo.toml and only the four Norn workspace package version records in Cargo.lock to0.1.0-preview.6. No third-party dependency version, source, checksum, feature or lock resolution changes are authorized by this row. Update docs/release-notes/UNRELEASED.md to distinguish preview.6 candidate R1-R3 from the actually installed preview.5 source05109e6f6294c9af6f58657722dc7e968857b721. Use its recorded install/hash and local-check evidence when correcting stale preview.5 wording. Preserve historical failed/interrupted attempts with their own scope; baseline6378 tests and prior installation do not verify the new candidate. Keep candidate explicitly uninstalled/unverified until actual final checks and installation receipt exist. R4 scroll/latest and selected-agent view/message remain pending; do not imply those ship with R1-R3. No tag, public release, main merge, exact205 or Fable pass follows from a prerelease version or local preview. Root runs final fmt, strict workspace/all-target/live-feature Clippy, AST and500 production-line checks, relevant unit and actual idle/active/resize/restart tests on the composed final bytes. Preserve all required source/selection/grapheme/keyboard/draft and publication assertions and record actual failures; no bypasses or borrowed verdicts. For an authorized successful local update, verify build source identity, installed --version and binary SHA, preserve and verify rollback binary/receipt, then invoke macOS say with exact text Norn updated and record its exit. Record the actual source/binary/check hashes and install time only after completion. Local preview authority remains separate from main-landing exact-commit 205 and Fable requirements.

### D-NUI004-INSTALLED-PREVIEW6: Recorded local preview.6 installation

> **Status:** locally_verified_installed_main_205_Fable_pending

norn 0.1.0-preview.6 was installed locally at 2026-09-06T18:11:46.211489+10:00 from d4913e67cdb4fba49ff3e4bf8a6c3656c532f7e8; executable SHA256 14f7acae36de6125a9e3993c7d826e3cebd8bc3b2eadc29f8d652b1d1c04dd62. NUI-004 R1-R3 provide the one-cell divider, F7/F8/F9 pane controls, displayed-text drag/copy and persisted exact Shift+Enter with F10/button cycling. Final matching-manifest local checks passed: 6,393 standard workspace/all-target tests across 39 suites, zero failed/ignored; harnessless channel stdio 14 and channel TUI 4; eight doctests; strict workspace/all-target/live-feature Clippy, fmt and release build. Bounded composed source policy06/07 is bound by install-policy-preflight.json. The prior binary is retained for rollback; root confirms macOS say Norn updated succeeded. R4 scrolling/Latest is written, saved and paused uncompiled in /private/tmp/norn-scroll-latest, excluded from preview.6. Agent target viewing/messaging, remembered auxiliary content, editable Work/Session panels and hosted attach remain pending. No main landing, exact-commit 205, Fable or public release is claimed. Final bounded policy07 receipt: /private/tmp/nui-004-proof/source-policy-07/final-summary.json (SHA256 85fc083d9db85df43c48b7d97d4dbba741e4188c9fdaf7d43115eab99cf9d028); all 23 Rust and three metadata files match installed d4913e6.

### D-NUI004-PREVIEW7-LOCAL: Resumed scrolling as a focused local preview

> **Status:** registered_no_new_verification_or_install

Parent relays explicit user authorization to resume and install scrolling for hands-on local testing after compile, focused scrolling and code-quality checks, preserving rollback. This does not waive full release/main, venue205 or Fable requirements. Preview.7 remains uninstalled. Root alone owns source/Cargo and the existing R5 metadata wall; all R4 acceptance, 14 modify paths and five create paths including screen_state.rs are preserved. Installed preview.6 d4913e6 and its final checks are not transferred to R4. Configurable shortcuts await their own named proposal.

### D-NUI004-VIEW-SHORTCUTS: Editable exact view bindings through one frontend settings owner

> **Status:** registered_implementation_pending

Provide editable exact view bindings at tui.input.bindings, defaulting to Option+P/D/A/S plus existing function-key aliases. Use one validated registry for dispatch, commands, persistence and hints; preserve existing input/selection/popup/provider semantics. The user accepts Option or Control shortcuts and requires editable settings. Declared defaults are Option+P pane toggle, Option+D Diff, Option+A Agents and Option+S send-policy cycle, retaining F7/F8/F9/F10 aliases. Existing F2-F6 actions become configurable through the same registry; preserve Shift+F6 reverse focus. Option is the delivered ALT modifier. Do not change global composer Enter/ShiftEnter/AltEnter send-policy defaults. Use tui.input.bindings as action -> array of exact stroke strings. An absent action inherits its declared defaults; an explicit [] unbinds that action including its aliases. Validate the complete effective registry atomically before publishing it. Unknown action/field names, wrong types, invalid strokes and conflicts are refused with named action/settings paths; never partially mutate live settings or silently discard a binding. Reuse the existing Iridium StrokePattern parser without new dependencies or Iridium/core changes. Restrict accepted bindings to one exact Char or F1..F12 stroke with explicit Ctrl/Alt/Shift modifiers where present. Reject wildcards/Any patterns, sequences/multi-stroke input, ambiguous Meta aliases and unsupported modifier forms, bare typing and reserved host/editor conflicts. Do not reinterpret SUPER as Control using the editor bridge. Compare exact delivered Termina code/modifier facts; modifiers not in a binding do not match it. After parser normalization reject duplicate effective strokes, including collisions with inherited defaults, and identify both action paths. Reject reserved cancellation, submit/newline, popup/editor and host-control conflicts rather than stealing their ownership. Cancellation/submission/paste/selection/delivery and popup routing remain unchanged. One Press may dispatch; Repeat and Release do not mutate or submit. Compile the validated effective bindings once when settings load or change. Dispatch uses an exact-key lookup without per-key allocation, reparsing or scanning the full keymap. Reuse existing startup raw settings; add no filesystem reads, locks or file creation merely to initialize shortcuts. Persist overrides inside the existing input-owned frontend preference projection and shared settings/CAS writer. Preserve the current User automatic-save working assumption, temporary run scope, explicit local scope, whole-tui-layer precedence, conflicts and durability outcomes. Reuse the single pending save owner; no parallel writer, polling, new queue or extra startup/service/MCP activation. Provide /view keys (and /view keys list) for the actual effective registry, /view keys set <action> <stroke>... to replace one action array, and /view keys clear <action> to publish [] for that action. Build and validate a complete candidate before applying any change. On rejection preserve the previous effective map and preferences. Accepted settings edits and later persistence errors remain distinct and visible through the existing error/status surface. Render help and composer hints from the effective validated registry, so Option/Control replacements and unbound actions are truthful. Preserve current chrome row count, composer width, clickable send-policy control, reserved Latest geometry and clipping/hit bounds. Do not retain a hardcoded F10 send-key hint after its binding changes. Existing commands keep their semantics and aliases. channels_compatibility owns the R6 implementation. Only the three newly created, unreferenced files may be authored after registration; root retains all existing/shared R4 source files until the initial TUI test run completes and root explicitly transfers them. No concurrent writers. Preserve root R4 Latest geometry tests in composer_tests.rs and scrolling/selection tests in retained_workspace.rs. Root alone owns Cargo, dependencies, builds and installation.

### D-NUI005-DETAILS: Shared per-item detail policy and readable lifecycle styling

> **Status:** implemented_installed_bounded_validation_open_findings

Use one per-item detail policy across retained painting and navigation: Context/compaction details remain collapsed until explicitly expanded, independent of row selection. Restore readable tool header/body lifecycle colours and backgrounds without changing original-body selection or the established layout. A shared typed per-item policy drives both visible groups and navigation row counts. Selecting a Context/compaction item does not expand its potentially large body; only the existing explicit expansion control does. Preserve compact labels, full retained metadata and original mapped body access when expanded. Apply lifecycle colours from actual ToolState/result_state, never from text searches. Keep supplied tool name plus description. Expanded detail styles and selected-row background must remain legible while preserving Markdown/source styles, original mappings and display-selection scope. Frame encoding handles selected background and the existing supported colour modes, including 256-colour background, with unchanged bounds and changed-cell publication semantics. Preserve wide/combining glyphs and no-paint behavior; no second terminal writer. Tests cover Context collapsed/expanded independent of selection, matching paint/navigation row counts, tool lifecycle/expanded colours, original source map preservation and actual frame background encoding. New transcript_items_tests.rs may hold these tests; preserve existing assertions. Preserve exact source/BodyRef/selection maps and typed retained rows; no ANSI roundtrip, global-height rewrite or whole-body parse on every paint. Keep each production file at most 500 nonblank/noncomment lines, module declarations thin, no suppressions, ignored tests, unwrap/expect/panic variants or swallowed Results. Root alone runs Cargo/builds and integration. Record actual formatter, strict Clippy, bounded AST/LOC and meaningful targeted/runtime checks; no unrun full-suite, venue or Fable success is inferred. The existing header click currently toggles only Tool. Extend mouse.rs through the same explicit expansion owner to Context and producer-bound notification ExternalInput using R3 typed binding facts; do not collapse all ExternalInput or ordinary MCP/user content. No XML/text-prefix trust. Preserve source/pane/publication hit identity and original selection behavior. Compact tool rows omit repetitive description-unavailable prose when absent; expanded details retain the explicit missing-description fact. Display actual result state without pretending an invocation was observed or matched. Supplied names/descriptions and unavailable identity remain truthful. Use the existing public nearest_256 helper for colour conversion; no style.rs edit is registered. The actual expansion action in app/view_actions.rs must use the same per-item default predicate as rendering/navigation: only Tool inherits global expanded_tools; Context and explicitly bound notification details default collapsed. With global tool details enabled, the first explicit Context click/toggle must open it and the next must close it. Preserve explicit per-item overrides and Tool behavior. Regression coverage may live in the already registered transcript_items_tests.rs or inline in view_actions.rs; no existing view_actions_tests.rs file was found, so no new separate test path is registered.

### D-NUI005-ASSOCIATION: Bind accepted canonical tool calls to the exact publication attempt

> **Status:** implemented_installed_bounded_validation_open_findings

During accepted reconcile_attempt, bind every canonical invocation in that owner-minted accepted HistoryRecord to its exact ticket AttemptKey, even when no provisional rows were observed. Insert/update through the item index so execution-scoped result lookup finds the pending call. An accepted receipt arriving before buffered call events still associates its canonical invocation with the exact ticket AttemptKey; the later execution-scoped result completes one described tool item. Page-first/receipt and live-first order remain idempotent. Refuse conflicting existing attempt ownership. Never associate by tool name, description, content, arrival order or a guessed call identity. Reused call IDs across attempts/executions remain isolated; ambiguous execution-scoped results remain explicitly refused. Preserve source-bound item/body identities and every index membership through the existing insertion owner. No extra transcript store, fabricated wrapper or provider/tool authority change. Regression tests cover receipt-before-live, page-first/receipt, existing live-first order, reused IDs across attempts/executions and conflicting/ambiguous ownership. Preserve exact source/BodyRef/selection maps and typed retained rows; no ANSI roundtrip, global-height rewrite or whole-body parse on every paint. Keep each production file at most 500 nonblank/noncomment lines, module declarations thin, no suppressions, ignored tests, unwrap/expect/panic variants or swallowed Results. Root alone runs Cargo/builds and integration. Record actual formatter, strict Clippy, bounded AST/LOC and meaningful targeted/runtime checks; no unrun full-suite, venue or Fable success is inferred. Bind all canonical invocation rows in the accepted HistoryRecord, including when the provisional observation contained only generic text and no tool fragments. Prevalidate any different existing AttemptKey before mutation, then stamp/reinsert canonical rows through ordinary reconciliation/index ownership. Preserve existing missing-parent and reused-call ambiguity tests.

### D-NUI005-CHECKPOINT: Record one checked version for integration and local installation

> **Status:** local_main_install_checkpoint_closed_open_findings

Root records the actual repair version in the existing workspace metadata/release notes and uses that same checked source/version for the user-authorized main integration and local installation. No docs author edits manifests or claims a merge/install before receipts. Root alone updates workspace.package.version and the four Norn workspace package version records in Cargo.lock if a repair version is assigned; no third-party dependency changes are authorized by this metadata row. Do not guess a version in registration. Preserve historical attempts and installed checkpoints; release notes distinguish actual source features, measured checks and installation. Required checks precede integration/install. The same checked version is merged and installed as instructed by the user; no intermediate version mismatch or invented success. Record actual source/manifest/check results, Git integration result, installed --version and SHA256, verified rollback and macOS say Norn updated outcome. Explicit user main-merge authority is recorded; it is not an invented venue205 or Fable pass. Unrun venue/review work stays explicit. Preserve exact source/BodyRef/selection maps and typed retained rows; no ANSI roundtrip, global-height rewrite or whole-body parse on every paint. Keep each production file at most 500 nonblank/noncomment lines, module declarations thin, no suppressions, ignored tests, unwrap/expect/panic variants or swallowed Results. Root alone runs Cargo/builds and integration. Record actual formatter, strict Clippy, bounded AST/LOC and meaningful targeted/runtime checks; no unrun full-suite, venue or Fable success is inferred.

The local preview.8 environment checkpoint is closed: norn 0.1.0-preview.8 was installed on 7 September 2026 at 16:11:08 Melbourne from main/origin/main commit 22748bce289811b82229a4dd193d7ead48819f00. Installed SHA256 68637cdaa925f25bfc16bb8bb41932f696936c2cb69f31131aa81f34a48e6fea matches the checked artifact; previous preview.7 is retained and hash-verified for rollback. release-final-3, clippy-clean-source and fmt-final-4 passed on unchanged final source SHA256 fa3cb41cd30e024fcecf89e9a5812a1386e21d9e6279b5e5ffe6f95267b41e5f. This closes local delivery, not policy or venue/review clearance.

Validation limits remain explicit: the first library suite reported 6,192 passes with three files changed during compilation. The repeat stopped at 4,701 core passes and one Bash output failure; an isolated exact retry passed, without making the repeat all-green. Ten retained-workspace PTY tests passed with only core borrowed-JSON source drift and unchanged UI files. These are bounded checks, not a fresh all-target/full-suite verdict. The unchanged Bash drain-cutoff finding, inherited AST debt, exact-commit venue205 proof and Fable review remain open.
...
### D-NUI005-NOTIFICATIONS: Present producer-bound notifications without exposing raw audit records by default

> **Status:** implemented_installed_bounded_validation_open_findings

Bind delivered notifications to the actual appended UserMessage with explicit optional user_event_id in the Custom delivered audit. Project typed compact notification facts; retain raw audit/framed body bytes lazily and preserve unbound input as untrusted ordinary Input. Serialize optional additional Custom delivered user_event_id from the exact actual appended UserMessage. Do not change the public lifecycle enum shape or rewrite existing durable records; no invented event sequence or inferred source identity. HistoryRecord carries compact notification facts, retaining explicit binding when forming projection metadata and raw audit body capability. Typed Message live facts and committed delivered audits become Metadata and deduplicate only through exact phase/message_id/recipient identity. A UserMessage with the explicit admitted producer binding becomes ExternalInput with a compact sender/process/watch summary. Original framed bytes remain lazy BodyRef content for explicit detail, copy and export. Legacy records lacking that binding remain ordinary Input; never trust XML framing, a prefix, plausible sender prose or content matching. Invalid/missing provenance remains visible untrusted input. Use one deterministic source-bound reducer for out-of-order pages/live events and duplicate observation. No eager raw-body reads, whole-history scans, second transcript store or per-paint parsing. Source/view/item/body identities and unavailable ranges remain honest. Coordinate R1 with a precise typed notification/binding predicate; ExternalInput kind alone cannot collapse unrelated ordinary MCP input. Context and bound-notification toggles remain root-owned TUI paths, outside this row. Producer/body/projection regressions cover exact actual event binding, legacy and forged-prefix input, invalid provenance, process/watch/delivery summaries, duplicate/live/history order and lazy original-body preservation. No changed public lifecycle shape or unrelated execution policy. Preserve exact source/BodyRef/selection maps and typed retained rows; no ANSI roundtrip, global-height rewrite or whole-body parse on every paint. Keep each production file at most 500 nonblank/noncomment lines, module declarations thin, no suppressions, ignored tests, unwrap/expect/panic variants or swallowed Results. Root alone runs Cargo/builds and integration. Record actual formatter, strict Clippy, bounded AST/LOC and meaningful targeted/runtime checks; no unrun full-suite, venue or Fable success is inferred.

### D-NUI005-COMPACTION-CONTEXT: Show actual compaction progress and current context use

> **Status:** implemented_installed_bounded_validation_open_findings

Publish typed lifecycle progress around actually admitted compaction work and render truthful status in the existing composer row. Track current-context estimate separately from cumulative billing tokens and use the actual effective model context limit for used percentage. The core compaction_progress owner supplies a typed operation guard and begin/commit/fail/cancel boundaries around the actual admitted compaction/summarization scope. No speculative start, extra provider call, invented operation identity, new timer, cap, queue or storage. Preserve existing AgentCompaction accounting and compaction policy. Automatic compaction progress spans actual summarization awaits; cancellation/drop resolves its operation so a spinner cannot remain falsely active. Typed source/operation facts determine current progress. Root/child, stale operation, lag and terminal transitions cannot imply another execution is still compacting. Manual handle_compact is synchronous with no terminal writer or await during its commit. Record factual start/end around the existing work and present finish/failure at the next frame; do not claim an animated/cancellable manual summary, insert an artificial delay, or start another service to create visible motion. Use app/context_status.rs as the single UI status/current-context accounting owner. Do not display the summed lifetime billing ledger as current occupancy. Track source-proven current estimate separately; derive used percentage only with the actual effective context limit. Missing/stale numerator or undefined denominator is unavailable, never fabricated zero percent. Preserve billing totals and public controls. Initialize and invalidate the estimate/limit from existing runtime settings and actual model/session transitions, including new/resumed sessions and effective model changes. No secondary startup filesystem read, guessed model window or silent carryover from another source. Render actual active progress/context percentage in the existing composer/status row with the existing status cadence; no new global row, height change, polling or timer. Completion/failure/cancellation or lag uncertainty removes a false active indication. Root R1 separately controls Context body collapse and styles. Thread the typed progress variant through the declared core provider/reexport, session projection, CLI structured output and TUI notices/exhaustive consumers. Preserve safe typed metadata descriptions and existing output protocol behavior; no wildcard swallowing or private-body exposure. Focused tests cover actual blocked-summary progress, successful commit, failure and cancellation; separate context estimate versus cumulative accounting, unknown denominator, model/session invalidation and lag/terminal cleanup; existing-row spinner/percentage bounds and preserved composer layout. Preserve existing local-compaction and streaming-nudge assertions with explicit exhaustive variants. R3 review_child_context owns session_view/live.rs first. Obtain explicit handoff before editing that shared file; other R4 paths may proceed after registration. Root owns R1 rendering files and final integration/Cargo. Any further exhaustive caller or production-size split must be named and registered before editing; no rotation.rs or unlisted helper is authorized. Preserve exact source/BodyRef/selection maps and typed retained rows; no ANSI roundtrip, global-height rewrite or whole-body parse on every paint. Keep each production file at most 500 nonblank/noncomment lines, module declarations thin, no suppressions, ignored tests, unwrap/expect/panic variants or swallowed Results. Root alone runs Cargo/builds and integration. Record actual formatter, strict Clippy, bounded AST/LOC and meaningful targeted/runtime checks; no unrun full-suite, venue or Fable success is inferred.
...
- `crates/norn-tui/src/agents/status_line/retained.rs`
- `crates/norn-tui/src/agents/status_line/retained_tests.rs`
...
- `crates/norn-tui/src/app/context_status.rs`
- `crates/norn-tui/src/app/dispatch.rs`
- `crates/norn-tui/src/app/dispatch/finalization.rs`
- `crates/norn-tui/src/app/display_selection.rs`
- `crates/norn-tui/src/app/display_selection_tests.rs`
...
- `crates/norn-tui/src/app/render/agents.rs`
- `crates/norn-tui/src/app/render/agents_tests.rs`
...
- `crates/norn-tui/src/app/render/navigation.rs`
- `crates/norn-tui/src/app/render/navigation_tests.rs`
- `crates/norn-tui/src/app/render/screen_state.rs`
- `crates/norn-tui/src/app/render/transcript.rs`
- `crates/norn-tui/src/app/render/transcript_items.rs`
- `crates/norn-tui/src/app/render/transcript_items_tests.rs`
...
- `crates/norn-tui/src/app/view_actions/latest.rs`
- `crates/norn-tui/src/app/view_actions/latest_tests.rs`
...
- `crates/norn-tui/src/app/view_actions/shortcut_commands.rs`
...
- `crates/norn-tui/src/input/view_shortcuts.rs`
- `crates/norn-tui/src/input/view_shortcuts_tests.rs`
...
- `crates/norn/src/loop/compaction_progress.rs`
...
- `crates/norn/src/loop/runner/tests/local_compaction.rs`
...
- `crates/norn/src/session_view/notifications.rs`
- `crates/norn/src/session_view/notifications_tests.rs`
...
- `docs/design/norn-retained-tui/briefs/NUI-003.json`
- `docs/design/norn-retained-tui/briefs/NUI-003.md`
- `docs/design/norn-retained-tui/briefs/NUI-004.json`
- `docs/design/norn-retained-tui/briefs/NUI-004.md`
