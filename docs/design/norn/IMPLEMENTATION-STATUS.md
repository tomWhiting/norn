...
| 33 | OpenAI Responses API | `provider/openai/` | `OpenAiProvider` with SSE streaming, tool calling, Norn-owned Codex-compatible OAuth at `$NORN_HOME/auth/auth.json` (default `~/.norn/auth/auth.json`), and ChatGPT backend auto-detection. A generic `provider/openai_compatible/` provider also exists. |
