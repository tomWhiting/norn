...
- **R2.4** The app SHALL support an **annotation layer** ("road signs") over timelines: bookmarks, labels, and notes attached to event ranges, stored outside the immutable event log and never mutating it. Storage: haematite as canonical store recommended (research-verdict, Appendix A; pin `97d6545b`); write discipline owner-ruled (§10.1: observer reads + open-per-write cycling); fork semantics ruling still open. Facts binding on the M1 brief: (a) fork copies events verbatim — EventBase ids are preserved across fork, so annotation keys are `(session, event)` unless annotations are explicitly ruled global; (b) all UI reads ride haematite's lock-free `ReadOnlyDatabase` observer (committed-state-only; the single-writer lock governs mutation exclusively); (c) the annotation read layer SHALL retry typed observer misses (`MissingNode`/`MissingCommittedRoot` from concurrent prune) by re-deriving the root and re-reading — a stated requirement, not incidental handling — **subject to the three constraints below, which are binding.** Amended 2026-08-08 after the engine owner (Apollo Biscuit, haematite seat) reported, and this seat independently confirmed at the bytes of the local haematite checkout @ `2089609` / 0.8.0, that **the engine does not currently discriminate the fault this retry is predicated on**: `crates/haematite/src/tree/source_error.rs` flattens `Self::NodeGet { hash, .. } => TreeError::MissingNode { hash }`, discarding the source, so a genuine disk/filesystem I/O failure arrives **byte-identical to a benign concurrent-prune absence**. `load_node_with_source` separates them correctly one level down; `load_node` does not. Tracked upstream as haematite #31 (with sibling #56, carry the typed cause and delete the wildcard); **not landed as of 0.8.0**. Drafted naively, this requirement therefore instructs an implementer to build a loop that silently absorbs failing hardware — the exact silent-failure class this estate bans. Binding constraints:
  - **(c-i) The retry SHALL be bounded, and exhaustion SHALL be loud.** Exhaustion SHALL escalate as a typed error naming the read path; it SHALL NOT be reported as "not found", and SHALL NOT be collapsed into an empty annotation set. Correct after #31 lands as well as before it, so this is not throwaway work.
  - **(c-ii) The retry predicate SHALL match exhaustively, with NO wildcard arm.** `TreeError`, `ObserverError`, `WalError` and `StoreError` are plain `pub enum`s — **none carries `#[non_exhaustive]`** (confirmed at `tree/cursor.rs:11`, `db/observer.rs:196`, `wal/buffer.rs:48`, `store/disk/error.rs:9`). A `_ =>` arm here would reintroduce, one layer up, precisely the swallowing construct #56 exists to delete. An exhaustive match means that when #31/#56 land and the fault taxonomy moves, **our build fails loudly** rather than the predicate silently ceasing to match and converting benign prune misses into hard errors. **That build break is required behaviour, not a regression** — if the taxonomy changes and our build does *not* break, that is itself the defect signal.
  - **(c-iii) "Retry on `MissingNode`" is underspecified and SHALL NOT be implemented from that phrasing.** `MissingNode` is six distinct variants across six enums (`tree::cursor`, `tree::diff`, `branch::merge`, `branch::prune`, `sync::merge`, `sync::handoff_merge`); `ObserverError` carries none of them directly — it wraps `Config`/`Store`/`Wal`/`Tree`/`Ttl`/`ShardOutOfRange`, so a miss arrives nested, and *which* nesting depends on the read path taken. `MissingCommittedRoot` is a `WalError` variant. The M1 brief SHALL carry the concrete match arms; an implementer given only the phrase above cannot write this correctly.
  - **(c-iv) Cross-estate pre-break check — a standing obligation, recorded here because two private notes are not a mechanism.** The haematite seat (Apollo Biscuit) and this seat agreed 2026-08-08 that when upstream #56 is ready, the branch and SHA are handed to norn **before** they reach haematite's main and **before any release**, norn is built against them, and the result is read as follows, agreed in advance in both directions: **our build breaks ⇒ the change is working**, and the shape of the break is learned while it is still cheap to change; **our build does NOT break ⇒ that is the finding, not reassurance** — either the change did not move the taxonomy it was meant to, or our match is not exhaustive where we believe it is; in both cases stop and look, do not ship. The gate is a release, and a crates.io release is irreversible in the way that matters (there is no unpublish; yank does not delete), so a control that runs only afterwards can detect but never prevent. **Disclosure, so no future reader mistakes care for robustness: no harness or CI gate enforces this on either side.** It is carried by the haematite release runbook as a publish precondition, by this clause, and by both seats' durable notes — and by nothing else. Wake phrase, recorded literally by both parties: **`pre-break check, #56`**. If this obligation matters enough to survive personnel and context turnover, it eventually wants a real mechanism; until then it wants both seats to re-read their own notes.
...
1. Annotation storage — **(a) fork semantics STILL OPEN** (global-by-event / snapshot-inherit-then-diverge / session-local). **(b) write discipline OWNER-RULED 2026-07-24:** reads via lock-free `ReadOnlyDatabase` observers; writes via **open-per-write cycling with bounded retry on the typed `DataDirLocked`** — the pattern haematite's A4 lock was explicitly built to make safe (reopen-after-drop guaranteed; contention fails loudly, never blocks or corrupts; kernel releases the lock on process death incl. kill -9; open cost is noise at human annotation rates). Haematite is multi-reader/multi-writer **in-process** (Send+Sync `&self` API over per-shard actors, soak-tested in `tests/multi_reader_writer.rs`); cross-process one-live-writer is deliberate A4 embedded-engine posture (SQLite/LMDB/RocksDB class). Escalate ONLY if open latency actually bites, and then to the sanctioned shapes — the haem daemon direction (double-fork semantics reserved in LOCKFILE-SEMANTICS.md; ops-console/haem MCP as the nascent broker surface) or replication between stores — never a transaction-scoped multi-process write API (surgery against the engine's in-memory serve-authority design). **(c) ENGINE-OWNER REVIEW 2026-08-08 (Apollo Biscuit, haematite seat) — three amendments to the above:**
   - **The A4 lock is settled as a *correctness boundary* and a placeholder as the *concurrency answer*, and the distinction is the answer.** It is not conservatism: each writer process runs its own shard actors over the same WAL files, and serve authority (`owner_stamps` — `live_epoch`/`seq`) is deliberately in-memory and this-lifetime-only, so two writer processes over one dir corrupt each other. The lock does not relax. **Annabel ruled 2026-07-24, controlling, that multi-process concurrent read AND write is a product requirement, not an edge case** — "we realise concurrency only in-process" is explicitly not acceptable, and **cycling is explicitly not an acceptable *general* answer**. Concurrency therefore arrives *beside* the lock, via the daemon (`HAEM-SERVE-DIRECTION.md`, status **PAPER ONLY, no implementation authority**, awaiting Waffles' charter). This spec SHALL NOT design against a daemon that has no authority to exist.
   - **SCOPE, and the question that governs whether the (b) ruling carries R2.4:** cycling is blessed for **rare, human-rate writes**, which is what R2.4 as specced is — bookmarks, labels and notes placed by a person through the app UI. **R2.4 names no agent writers.** Curation agents annotating per-event belong to the session-vision braindump, not this milestone; that case is agent-rate and falls squarely inside what Annabel ruled cycling insufficient for. **If agent annotation is ever brought into scope, R2.4 SHALL be re-sized and escalated to Waffles (charter) and Annabel (requirement) before implementation — not absorbed.**
   - **Correction to the escalation shapes named above:** `OPS-CONSOLE.md` is DRAFT r1 and a **read-only contract** — using it as a write broker is a new thing, not an escalation of an existing plan, and whether `haem mcp` and `haem serve` are one surface family or two is open question 6 in the serve doc, unresolved. **Replication SHALL NOT be used as the local answer**: it forces conflict-policy semantics onto data that never needed to diverge, SYNC-CARRIER piece 2 is held, and the LWW policy in `sync/merge.rs` carries a written DANGER note (it silently drops divergent writes) and is fenced out of production paths. The exclusion of a transaction-scoped multi-process write API is **confirmed correct and is independently the engine owner's documented §5 position**.
   - **PIN DRIFT:** the `97d6545b` pin cited in R2.4 is **208 commits behind** and was crate 0.6.1; 0.6.2, 0.7.0 and 0.8.0 have since shipped (0.8.0 published 2026-08-07). `branch::merge` is **deleted** in 0.8.0 in favour of `merge_branches(store, MergeTransaction)`, and the beamr floor moved to 0.16.3. Haematite releases SHALL be identified by `.cargo_vcs_info.json` inside the published `.crate`, **never by git tag** (only `v0.8.0` is tagged; 0.6.2 and 0.7.0 shipped untagged, so `git tag --contains` under-reports); haematite does not track `Cargo.lock`. No bump is proposed here — the human-rate/agent-rate answer may change what is needed from the engine. Note also that `HAEM-SERVE-DIRECTION.md` §3 cites `branch/commit.rs::merge`, deleted in 0.8.0: read it for reasoning, not line numbers.
