...
| Finding | Baseline proof | Candidate regression(s) | State before final Gate C |
|---|---|---|---|
| `SEC-06` | Source proof at review lines 238 onward: credentials/claims/metadata/authority bodies can escape diagnostics. | Redacted `Debug` tests across OAuth types/JWT/PKCE/manager and provider config; refresh authority-body test; debug response-metadata test; TUI prompt/history `Debug` sentinels added by the artifact correction. | Candidate present. |
