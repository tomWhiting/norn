...
[`P1 Gate A READY review`](2026-07-15-p1-gate-a-review.md), with the explicit
generated-include correction below awaiting foundation re-review. It defines
P1's shared repository policy but does not claim that the implementation
exists.
...
Compile-test fixtures are test-only only through a closed `trybuild` harness
proof. The harness must be an auto-discovered integration target at exactly
`tests/<target>.rs` or `tests/<target>/main.rs`; any explicit `[[test]]` table
rejects the proof. Its package must declare `trybuild = "1"` as a direct string
under `[dev-dependencies]`, its exact local package entry in `Cargo.lock` must
name a direct `trybuild` dependency, and the lock must contain exactly one
crates.io `trybuild` package at version `1.0.117`, source
`registry+https://github.com/rust-lang/crates.io-index`, and checksum
`0710d4dfbeae4f9c390baa784c49858a7468fa433f3fe5d0ec5ebef651cf59f9`.
Root `[patch]`/`[replace]` authority and repository `.cargo/config` or
`.cargo/config.toml` `paths`, `patch`, `source`, or `registries` authority make
the dependency proof invalid.

Its only non-comment root syntax is exactly one ordinary, parameterless
`#[test] fn <identifier>()` with no visibility,
`async`, `const`, `unsafe`, ABI, generics, or return type. Its body starts with
one immutable untyped
`let <binding> = trybuild::TestCases::new();` and then contains only one or more
direct `<binding>.compile_fail(<literal>)` or
`<binding>.pass(<literal>)` statements. Imports, aliases, helper functions,
modules, closures, control flow, reassignment, indirect calls, dynamic
selectors, extra attributes, or any other syntax invalidate the proof.

Each selector is either one exact `tests/<subtree>/<file>.rs` path or the
terminal form `tests/<subtree>/*.rs`; no other glob syntax, empty component,
`.`/`..`, absolute path, backslash, control character, symlink/non-regular
entry, or package escape is admitted. Every selected source must belong to the
same deepest workspace package as the harness. A valid fixture root is walked
recursively through ordinary `mod`, literal `#[path]`, and literal `include!`
edges under a synthetic test-only reach mode with `cfg(test)` evaluated false,
so fixture-internal production-shaped code cannot hide behind `cfg(test)`.
Any source also reachable from a production root remains production. The
ratified P1 base has one qualifying harness selecting exactly the eleven
`crates/norn-macros/tests/ui/*.rs` fixture roots; base/current authority and
phase-lock evidence must preserve that exact classification or fail closed.
...
path/hash, complete repository input path/hash set, and output basename. The
base registry includes only
...
**Foundation correction (2026-07-15):** the Gate A candidate also required a
static owner and an output-schema label. Neither field had executable semantics:
the owner duplicated Git/review accountability, while the schema was checked
only for non-emptiness and interpreted nowhere. Both are removed rather than
placing arbitrary labels inside the exact-base digest. Exact invocation,
source/callsite/enclosing span, target, generator and complete input bytes, and
output basename remain bound; the workspace build validates the emitted Rust.
A future interpreted generated-output validator may introduce a versioned
schema identity through an explicit reviewed migration. This correction does
not alter the separate owner/due-phase requirements for temporary legacy debt.
...
Limits are hard: 200 production LOC for a source that is the exact crate root of
a discovered library, proc-macro, or binary target, and 500 for every other
production Rust file. Entrypoint classification is semantic, not basename
based: examples, build scripts, integration tests, benchmarks, and nested files
named `lib.rs` or `main.rs` use the 500-line ceiling. A production `mod.rs`
...
`policy/governance/legacy.toml` maps legacy LOC and prohibited-debt origin IDs
to owner, due phase, and remediation record. It is strict reviewed metadata.
The same sealed reviewed input first authors
`policy/governance/p1-reviewed.toml`, an immutable anchor in which every base
exception is active. That exact non-empty anchor and its normalized digest are
independently reviewed before the digest is compiled into the evaluator and
recorded in the phase lock. Callers cannot supply or replace the compiled
anchor identity. `policy/governance/legacy.toml` is the current successor: it
must preserve the anchor's complete identity set, owner, remediation record,
and other reviewed metadata, while permitting only an earlier due phase or an
`active` to `resolved` state transition. Removal, reactivation, later due
phase, or metadata drift fails closed.
The separate `policy/writer-families.toml` authority maps every generated
writer-operation origin to its technical artifact-family classification and
shared-primitive edges; generated writer data contains no human owner field.
An immutable origin entry is never deleted when debt is fixed. Current active
exceptions are derived as `origin - resolved`; a current file at/below its
limit or a removed debt occurrence becomes resolved. No new active exception
can be added.
...
`policy/phase-lock.json` pins the active phase (`P1`), the explicit Git object
format and base commit/tree, schema/analyzer versions, and normalized digests of
policy, governance, the combined built-in sink and writer-family authority, the
generated-include technical registry, the aggregate public-and-Codex contract
authority, the complete redaction registry, source-finding registry, and gate
command manifest. The CLI accepts no alternate base, phase, policy path,
scope, advisory mode, or exclusion option. The first P1 foundation commit and
its lock digest are recorded in the plan ledger and reviewed before later work.
...
Every captured/output artifact family has a versioned closed schema and string
grammar. Checked-in evidence-tool source is a separate reviewed-source family:
only the exact preregistered script and extractor paths are admitted; each file
must be UTF-8, pass the raw dangerous-shape scan, match its registered digest,
and pass the ordinary source lint and independent code review. Source code is
never accepted as a captured observation and P1 does not claim that arbitrary
Python, shell, or Rust source has the JSON/log string grammar. For captured
artifacts, the validator:
