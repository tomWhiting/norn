# DRAFT Claude SDK parity integration checkpoint — 6 September 2026, Melbourne

**This is a development checkpoint with two merge blockers. It is not approved for main, release or installation.** The checkpoint preserves the user's requested integration work while those findings are repaired. The original `codex/claude-sdk-parity` branch remains preserved at `8faa4c36ff35a5e75276479b4463f63c33aa7751`; main is not changed by this checkpoint.

Current registration: [NSP-001](../design/norn-claude-parity/briefs/NSP-001.md). Integrated base: `d27259cc1c218a44be0ce624d103b04d799dcf21`. Development branch: `codex/norn-claude-parity-integrated`; worktree: `/private/tmp/norn-claude-parity-integrated`. The draft commit has the integrated base and the preserved parity tip as parents. Its exact commit and push verification are recorded externally in `/private/tmp/norn-claude-parity-integration-proof/draft-checkpoint/result.json`.
...
The lifecycle finding is source-backed; no runtime reproduction or fixed dependency revision is claimed. A detached wrapper task retaining the permit would still lose the SDK's termination authority. Timed process polling and raw-PID killing are not accepted workarounds. The existing Runner working checkout has unrelated staged/unstaged work and was not modified.
...
The first Clippy pass uncovered an inherited no-effect trait coercion that a removed suppression had hidden. Its test-only repair now asserts the Claude backend through the Provider trait object. Only that assertion changed between the original targeted-test freeze and the final Clippy/fmt freeze. The draft documentation update changes no checked Rust, catalogue, manifest or lockfile bytes. All 35 staged paths stay within NSP-001; no unresolved source conflict remains.

The final bounded AST/LOC scan covers 22 changed Rust files with no new violation, forbidden attribute/call, over-500 result or mod.rs shape finding. The ten inherited underscore operations above remain explicit blockers; the scan is not a universal policy-clearance or battery receipt.

## Integrated behavior and limits

The reconciliation retains Astra/defaults, single-pass alias precedence, actual-route model preflight, child context provenance and later Claude terminal/malformed-payload handling. The provider and CLI declare the same `CatalogBackend::CLAUDE`; a catalogue route is only a hint when no explicit provider/profile/API shape is supplied. Explicit selection retains authority and validates its actual route's metadata.

`wrapped/execution.rs` owns the existing one-shot mode; `spawn_session` creates the persistent SDK mode. The uncited 128-character tool-name cap is removed while namespace grammar remains enforced. Explicit `None` and `Ultra` Claude efforts are refused before spawn. The Runner pin is `3231faadbecd7cddaeb4c9c4e00c206341c73456`; the unresolved lifecycle behavior at that exact pin blocks landing.

The persistent wrapper is a library execution mode. Standard Norn Provider turns use the model-only adapter, which refuses nonempty Norn tool schemas. This checkpoint supplies no full tool-enabled CLI Claude driver, Liminal session host or live attach/detach daemon. Fake CLI fixtures do not prove installed Claude model availability, entitlement or live wire compatibility.

Close both blockers, update the reviewed dependency revision, and obtain the required affected/full tests, rigorous review and exact-commit venue receipt before main landing. Keep this development checkpoint and the original parity branch until that accepted integration is complete.

Historical handoff follows unchanged; its dates, branch tips and Gate A/B claims describe the July candidate, not this draft.
