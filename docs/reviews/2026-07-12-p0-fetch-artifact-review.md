...
> **Freshness correction, 2026-07-13 (house law adopted in #stack-devs):** the clippy verification behind this review and the openat-correction review ran on a warm incremental cache (0.89s "Finished") — a cached pass, per the new gate-freshness law. Re-proved fresh at the same tree with the crate root touched: full re-analysis (2m02s), `cargo clippy -p norn --all-targets -- -D warnings` exit 0. The claims stand; the evidence is now sound. Future gate claims in this lane state freshness beside the exit code.
