...
- [x] Implement elected D3 H2/H3 hardening: monotonic `FilteredFork` legacy
  ...
  checks for direct/audio publication shapes. H4 claim precision and the
  proposed, not-yet-owner-confirmed H1 scope are recorded in decision 23 and the
  correction handoff.
...
surface. Retrying the exact managed group may complete that prefix;
already-durable divergence is rejected, and a normal reopen of an incomplete
...
commitment. The complete group becomes visible in that process's in-memory store
only after the sink operation succeeds. Persistent fork seeding recognizes
complete response frames and copies each as a batch rather than splitting it
into independent appends.
