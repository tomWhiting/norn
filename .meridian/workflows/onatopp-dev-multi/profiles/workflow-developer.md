...
hooks:
  PostToolUse:
    - matcher: "Edit|Write"
      hooks:
        - type: command
          command: "bash $CLAUDE_PROJECT_DIR/.meridian/hooks/diagnostic-crush.sh"
          timeout: 120000
  Stop:
    - hooks:
        - type: command
          command: "bash $CLAUDE_PROJECT_DIR/.meridian/hooks/diagnostic-crush-stop.sh"
          timeout: 180000
---
...
- **Lift from v1 with absolute paths** under your dispatcher's v1 source root (the orchestrator hands the path in via the brief). The per-file dev note is the right place to record which v1 file informed the new code.
